...
ClipboardGUI::ClipboardGUI(std::string history_path, bool enable_copy, QWidget *parent)
  ...
  _history_button->setGeometry(16, _button_padding, _button_width, _button_height);
  connect(_history_button, SIGNAL (pressed()), this, SLOT (handleHistoryButton()));
...
void ClipboardGUI::copyItem(int x) {
  ...
  if (_enable_copy) {
    ...
    outfile << _clipboard.getHistoryItem(x) << std::endl << "*-*-*-*-*-*-*-*-*-*" << std::endl;
    outfile.close();
  }
}
...
void ClipboardGUI::addClipboardItem(std::string item_data, int index) {
    ...
    connect(item, &ClipboardItem::copyButtonPressed, this, &ClipboardGUI::copyItem);
};
